...
async function updateProfile() {
    ...
    try {
        await userSync.updateUser(sessionUser.value.id, {
            name: newName.value,
        });
        newName.value = "";
        // toast.add({
        ...
        <div class="p-6 max-w-4xl mx-auto">
            ...
            <div class="mb-8">
                <h1 class="text-2xl font-bold mb-2">
                    ⚡ Electric SQL Sync - ShapeStream Pattern
                </h1>
                <p class="text-gray-600 dark:text-gray-400">
                    Using ShapeStream for sync events + usePgWorker for data
                    queries
                </p>
            </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    <div class="flex items-center gap-2">
                        <span
                            ...
                            :class="
                                pg.isReady.value ? 'bg-green-500' : 'bg-red-500'
                            "
                        />
                        <span
                            >PGlite:
                            {{
                                pg.isReady.value ? "Ready" : "Loading..."
                            }}</span
                        >
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        <span
                            class="w-3 h-3 rounded-full"
                            :class="
                                userSync.isSyncing.value
                                    ? 'bg-yellow-500 animate-pulse'
                                    : userSync.isUpToDate.value
                                      ? 'bg-green-500'
                                      : 'bg-gray-400'
                            "
                        />
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span
                            >Auth:
                            {{ loggedIn ? "Logged in" : "Logged out" }}</span
                        >
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        <span
                            class="w-3 h-3 rounded-full"
                            :class="
                                userCount > 0 ? 'bg-blue-500' : 'bg-gray-400'
                            "
                        />
            ...
            <UCard class="mb-6">
                <template #header>
                    <div class="flex items-center gap-2">
                        ...
                        <span class="font-semibold"
                            >Current User (Queried with usePgWorker)</span
                        >
                    </div>
                </template>
                ...
                <div v-if="currentUser" class="space-y-2">
                    ...
                    <div
                        class="mt-4 pt-4 border-t text-sm text-gray-600 dark:text-gray-400 space-y-1"
                    >
                        <p>
            ...
            <UCard>
                ...
                <div v-if="users.length > 0" class="space-y-2">
                    <div
                        ...
                        :class="{
                            'ring-2 ring-primary': u.id === currentUser?.id,
                        }"
                    >
                ...
                <div v-else class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-sm mt-2">
                        Sync is active - data will appear here.
                    </p>
                </div>
            </UCard>
            ...
            <UCard class="mt-6">
                ...
                <pre
                    class="text-xs bg-gray-100 dark:bg-gray-900 p-4 rounded overflow-auto"
                    {{
            ...
            <UCard class="mt-6">
                ...
                <div class="prose dark:prose-invert max-w-none text-sm">
                    <p>
                        This page demonstrates the new Electric SQL ShapeStream
                        pattern:
                    </p>

                    <ul class="list-disc pl-5 space-y-1">
                        <li>
                            <strong>useElectricSync</strong> - Subscribes to
                            sync events (onInsert, onUpdate, onDelete,
                            onUpToDate, onError)
                        </li>
                        <li>
                            <strong>usePgWorker</strong> - Pages query data
                            themselves using query(), queryOne(), or liveQuery()
                        </li>
                        <li>
                            <strong>Separation of concerns</strong> - Sync
                            events trigger re-queries, not data management
                        </li>
                    </ul>

                    <pre
                        class="bg-gray-100 dark:bg-gray-900 p-3 rounded mt-4"
                        v-pre
                    >
// Subscribe to sync events
